...
BOOL kuhl_m_kerberos_pac_to_infos(PPACTYPE pacType, DWORD pacLenght, PISID *sid, DWORD *rid)
{
	...
	for(i = 0; i < pacType->cBuffers; i++)
	{
		if(pacType->Buffers[i].ulType == PACINFO_TYPE_LOGON_INFO)
		{
			if(status = kull_m_pac_DecodeValidationInformation((PBYTE) pacType + pacType->Buffers[i].Offset, pacType->Buffers[i].cbBufferSize, &pValInfo))
			{
				if(sid && !*sid)
					if(len = GetLengthSid(pValInfo->LogonDomainId))
						if(*sid = (PISID) LocalAlloc(LPTR, len))
							RtlCopyMemory(*sid, pValInfo->LogonDomainId, len);
				if(rid && !*rid)
					*rid = pValInfo->UserId;
...
void kuhl_m_kerberos_pac_info_data(PPACTYPE pacType, DWORD pacLenght, EncryptionKey *AsRepKey)
{
	...
	for(i = 0; i < pacType->cBuffers; i++)
	{
		...
		case PACINFO_TYPE_LOGON_INFO:
			kprintf(L"*** Validation Informations ***\n");
		...
		case PACINFO_TYPE_CREDENTIALS_INFO:
			kprintf(L"*** Credential information ***\n");
...
void kuhl_m_kerberos_pac_info_data_credentialinfo(PPAC_CREDENTIAL_INFO pCredentialInfo, ULONG uCredentialInfo, EncryptionKey *AsRepKey)
{
	NTSTATUS ntStatus;
	DWORD i;
	OssBuf credInfoIn, credInfoOut = {0, NULL};
	PPAC_CREDENTIAL_DATA credentialData = NULL;
	PNTLM_SUPPLEMENTAL_CREDENTIAL ntlmCredential = NULL;
	credInfoIn.value = pCredentialInfo->SerializedData;
	credInfoIn.length = uCredentialInfo - FIELD_OFFSET(PAC_CREDENTIAL_INFO, SerializedData);
	if(AsRepKey && (AsRepKey->keytype == pCredentialInfo->EncryptionType))
	{
		ntStatus = kull_m_kerberos_asn1_crypto_encrypt(KERB_NON_KERB_SALT, AsRepKey, &credInfoIn, &credInfoOut, FALSE);
		if(NT_SUCCESS(ntStatus))
		{
			if(kull_m_pac_DecodeCredential(credInfoOut.value, credInfoOut.length, &credentialData))
			{
				for(i = 0; i < credentialData->CredentialCount; i++)
				{
					kprintf(L"  [%u] %wZ\n", i, &credentialData->Credentials[i].PackageName);
			...
			LocalFree(credInfoOut.value);
		}
		else PRINT_ERROR(L"Decrypt: %08x\n", ntStatus);
	}
	else
	{
		kull_m_string_wprintf_hex(credInfoIn.value, credInfoIn.length, 1 | (16 << 16));
		kprintf(L"\n");
	}
}